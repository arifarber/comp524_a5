    ...
    (let ([match-results (filter-map (λ (entry) (table-entry->result input entry))
      ...
      (if (empty? match-results)
        ...
        (let* ([longest-match-result (first (sort match-results > #:key first))]
          ...
          (if token
            (cons token (tokenize2 remaining-input))
            (tokenize2 remaining-input)))))))

(module+ test
  (require (only-in rackunit
                    check-equal?))

  (check-equal? (tokenize2 "define") (list (token 'DEFINE #f)))
  (check-equal? (tokenize2 "") null)
  (check-equal? (tokenize2 " \n\t\r ") null)
  (check-equal? (tokenize2 ";; comment1") null)
  (check-equal? (tokenize2 ";; comment1\n7") (list (token 'INT 7)))
  (check-equal? (tokenize2 ";* comment2 \n *;\n8") (list (token 'INT 8)))
  (check-equal? (tokenize2 "\"\"") (list (token 'STRING "")))
  (check-equal? (tokenize2 "\"abc\"") (list (token 'STRING "abc")))
  (check-equal? (tokenize2 "if1") (list (token 'NAME 'if1)))
  (check-equal? (tokenize2 "1if") (list (token 'INVALID "1if")))
  (check-equal? (tokenize2 "9.8") (list (token 'FLOAT 9.8)))
  (check-equal? (tokenize2 "9") (list (token 'INT 9)))
  (check-equal? (tokenize2 "1a") (list (token 'INVALID "1a")))
  (check-equal? (tokenize2 "123\"456\"") (list (token 'INVALID "123\"456\"")))
  (check-equal? (tokenize2 "\"123\"456") (list (token 'INVALID "\"123\"456")))
  (check-equal? (tokenize2 "abc\"def\"") (list (token 'INVALID "abc\"def\"")))
  (check-equal? (tokenize2 "\"abc\"def") (list (token 'INVALID "\"abc\"def")))
  (check-equal? (tokenize2 "\"\\\\\"\"") (list (token 'INVALID "\"\\\\\"\"")))
  (check-equal? (tokenize2 "\"a\\\\nbc\"") (list (token 'STRING "a\\nbc")))
  (check-equal? (tokenize2 "\"\\\"\"") (list (token 'STRING "\"")))
  (check-equal? (tokenize2 "\"\\n\"") (list (token 'STRING "\n")))
  (check-equal? (tokenize2 "\"\\\\\"") (list (token 'STRING "\\")))
...
print(+(\"5! is \" factorial(5)))")
  ...
  (check-equal? (tokenize2 example-program) example-program-tokens))